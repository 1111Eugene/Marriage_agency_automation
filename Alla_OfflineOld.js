...
options.addArguments([
	"--user-data-dir=d:\\SELENIUM\\Girls\\Alla_58_offline\\",
...
const driver7 = new Builder().forBrowser("chrome").setChromeOptions(options).build();
...
async function getToOnline() {
	...
	try {
		...
		await driver7.get("http://***");

		try {
			await driver7.findElement(By.xpath("/html/body/div/div[4]/div/div[2]/div[2]/button")).click();
			await driver7.sleep(2000);
		} catch {
			// click online button
			// await driver7.wait(until.elementLocated(By.className("chat-online-icon")), 2000).click();

			// enable scroller window
			await driver7.wait(until.elementLocated(By.className("chat-list__in")), 2000).click();
			// scroll to the bottom
			let clickCount = 0;
			for (let i = 0; i < 200; i++) {
				await driver7.actions().sendKeys(Key.PAGE_DOWN).perform();
				await driver7.sleep(1120);
				if (clickCount % 30 === 0) {
					await driver7.wait(until.elementLocated(By.className("chat-list__in")), 2000).click();
				}
			}
			await driver7.actions().sendKeys(Key.HOME).perform();
			...
			while (count < minuscount) {
				await driver7
					...
					.click();
				await driver7.sleep(2000);
				let text = await driver7.findElement(By.className(`chat-field-info-text`)).getText();
				//
				//

				let time = await driver7.findElements(By.className("message-text-time"));
				for (let e of time) {
				...
				//
				//
				let Name = await driver7.findElements(By.className("chat-head-user-name"));
				let nameArray = [];
				for (let i of Name) {
				...
				let random = Math.floor(Math.random() * allaInvites.length);
				let random2 = Math.floor(Math.random() * greetings.length);

				//

				let link1 = await driver7
				...
				try {
					await driver7.findElement(By.className("btn-icon active"));
					if (linkList.indexOf(mainLink) === -1 && text.startsWith("You have")) {
						linkList.push(mainLink);
						// await driver7.executeScript(`window.open("https://${mainLink}",'_blank');`);
						// player.play("CHAT_alla.mp3");
					}
				} catch {
					if (text.startsWith("You have 1 message")) {
						await driver7.findElement(By.className("btn-icon")).click();
					} else if (text.startsWith("You have") && clock.includes(`${clockArray[0] >= 10 && "hours"}`)) {
						await driver7
							.findElement(By.name("note"))
							.sendKeys(`${greetings[random2]} ${firstName[0]}!:) ${allaInvites[random]}:)`, Key.ENTER);
					} else if (
		...
		await driver7.actions().sendKeys(Key.F5).perform();
	}
}
