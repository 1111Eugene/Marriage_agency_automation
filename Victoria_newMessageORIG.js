...
// WAIT UNTIL TEXT AUTHOR YOU ETC
				...
				if (actionText.startsWith("You have")) {
					if (!msgArray.includes(msgText)) {
						...
						if (chatsArray.indexOf(mainLink) === -1) {
							...
							await driver3.actions().keyDown(Key.CONTROL).sendKeys("a").perform();
							await driver3.actions().keyDown(Key.CONTROL).sendKeys("c").perform();

							// await driver3.sleep(1400);
							await driver3.executeScript(`window.open("https://${mainLink}",'_blank');`);
							console.log(msgText);
						}
					}
