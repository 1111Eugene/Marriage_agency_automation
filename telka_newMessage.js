...
options.addArguments([
	"--user-data-dir=d:\\SELENIUM\\Girls\\svitlana_message_1\\",
...
async function Active_newMessage() {
	...
	for (;;) {
		...
		try {
			...
			await driver3.wait(until.elementLocated(By.className("dialog-item")), 10000).click();
			...
			let link1 = await driver3
				.wait(until.elementLocated(By.xpath(`/html/body/div[1]/section/div/div/div/div[2]/div/div[1]/div/div[3]/a`)), 10000)
				.getAttribute("href");
			...
			let actionText = await driver3
				.wait(until.elementLocated(By.xpath(`/html/body/div[1]/section/div/div/div/div[2]/div/div[2]/div/div[1]/div[2]/p`)), 15000)
				.getText();
					...
					if (!words.startsWith("You sent") && !words.startsWith("You liked") && !msgText.startsWith("You:")) {
						if (!msgArray.includes(msgText)) {
							...
							if (chatsArray.indexOf(mainLink) === -1) {
								...
								// write the message
								await driver3.wait(until.elementLocated(By.name("note")), 5000).sendKeys(`${greetings[random2]} ${firstName[0]}!:)`);
				...
				if (actionText.startsWith("You have")) {
					if (!msgArray.includes(msgText)) {
						...
						if (chatsArray.indexOf(mainLink) === -1) {
							...
							// write the message
							await driver3
								.wait(until.elementLocated(By.name("note")), 5000)
