...
options.addArguments([
	"--user-data-dir=d:\\SELENIUM\\Girls\\Kate_31\\",
...
async function getToOnline() {
	...
	try {
		...
		try {
			...
			// click online button
			await driver.findElement(By.className("chat-online-icon")).click();

			// enable scroller window
			await driver.findElement(By.className("chat-list__in")).click();
			...
			while (count < minuscount) {
				await driver
					...
					.click();
				// wait a bit
				await driver.findElement(By.className(`chat-field-info-text`)).getText();
				// let text = await driver.wait(until.elementisLocated(By.className(`chat-field-info-text`)), 2000).getText();
				//
				//
