...
options.addArguments([
	"--user-data-dir=d:\\SELENIUM\\Girls\\Irina_Messages_1\\",
	...
	for (;;) {
		try {
			...
			try {
				...
				await driver3.findElement(By.className("dialog-item")).click();

				// last message text
				await driver3.sleep(3000);
				let msgText = await driver3.findElement(By.className("dialog-user-text")).getAttribute("innerText");
				...
				let words = await driver3.findElement(By.className("dialog-user-text")).getAttribute("innerText");
				...
				//
				// GET THE NAME
				let Name = await driver3.findElements(By.className("chat-head-user-name"));
				let nameArray = [];
				for (let i of Name) {
